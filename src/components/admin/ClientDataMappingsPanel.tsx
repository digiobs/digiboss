...
export function ClientDataMappingsPanel({ clients }: { clients: ClientLite[] }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (form.id) {
      const { error } = await (supabase as unknown as { from: (table: string) => Record<string, unknown> }).from("client_data_mappings").update(payload).eq("id", form.id);
      if (error) {
    ...
    const { error } = await (supabase as unknown as { from: (table: string) => Record<string, unknown> }).from("client_data_mappings").insert(payload);
    if (error) {
  ...
  const handleDelete = async (id: string) => {
    const { error } = await (supabase as unknown as { from: (table: string) => Record<string, unknown> }).from("client_data_mappings").delete().eq("id", id);
    if (error) {
  ...
  const syncFromClientConfigs = async () => {
    ...
    const { error: upsertError } = await (supabase as unknown as { from: (table: string) => Record<string, unknown> })
      .from("client_data_mappings")
